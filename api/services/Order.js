...
var exports = _.cloneDeep(
  require("sails-wohlig-service")(
    schema,
    "user  products.product courierType returnedProducts.product",
    "user",
    "createdAt",
    "desc"
  )
...
var model = {
  ...
  hdfcPaymentGateway: function(data, resp) {
    ...
    var body =
      "tid=1234&merchant_id=150530&order_id=123456789&currency=INR&amount=20&redirect_url=" +
      env.realHost +
      "/api/order/gatewayResponse&cancel_url=" +
      env.realHost +
      "/api/order/cancelResponse&language=EN";

    var workingKey = global.workingKey;
    var accessCode = global.accessCode;
    var encRequest = ccav.encrypt(body, workingKey);
    var reqdata =
      "encRequest=" + encRequest + "&" + "access_code=" + accessCode;
        ...
        if (!_.isEmpty(cart)) {
          ...
          if (data.selectedDiscount) {
            if (data.selectedDiscount.selectedDiscount) {
              order.selectedDiscount =
                data.selectedDiscount.selectedDiscount._id;
            }
          }
          ...
          for (var idx = 0; idx < cart.products.length; idx++) {
            ...
            var orderData = {
              ...
              discountPriceOfProductApplied:
                product.discountPriceOfProductApplied,
            ...
            if (orderData.discountAmount > 0) {
              order.totalDiscount =
                order.totalDiscount +
                _.round(orderData.discountAmount) +
                _.round(orderData.discountPriceOfProductApplied);
              order.totalAmount =
                order.totalAmount - _.round(orderData.discountAmount);
            } else {
              order.totalDiscount =
                checkNaN(order.totalDiscount) +
                _.round(checkNaN(orderData.discountAmount)) +
                _.round(checkNaN(orderData.discountPriceOfProductApplied));

              order.totalAmount -= _.round(checkNaN(orderData.discountAmount));
            }
          }
            ...
            GiftCard.findOne({
              ...
              if (err) {
                ...
                if (!_.isEmpty(voucher)) {
                  ...
                  order.totalAmountgiftVoucher += _.round(
                    orderVoucher.giftAmount
                  );
          ...
          if (data.selectedDiscount) {
            ...
            order.amountAfterDiscount =
              data.selectedDiscount.grandTotalAfterDiscount;
            order.discountPriceOfProductApplied =
              data.discountPriceOfProductApplied;
          }
          ...
          Order.saveData(order, function(err, data1) {
            ...
            if (err) {
              ...
              Order.findOne({
                ...
                .deepPopulate(
                  "products.product products.product.size products.product.color"
                )
                .exec(function(err, order) {
                  ...
                  if (cart.giftVoucher.length > 0) {
                    Coupon.saveData(couponGift, function(
                      err,
                      couponGiftReceived
                    ) {
                      if (err) {
                  ...
                  if (allData.selectedDiscount) {
                    if (allData.selectedDiscount.selectedDiscount) {
                      if (
                        allData.couponData &&
                        allData.selectedDiscount.selectedDiscount.discountType.toString() ==
                          "59f06bc7647252477439a1e4"
                      ) {
                        ...
                        couponDataToProcess.cAmount =
                          allData.selectedDiscount.selectedDiscount.xValue;
                        couponDataToProcess.startDate = new Date();
                        couponDataToProcess.endDate = new Date(
                          +new Date() + 365 * 24 * 60 * 60 * 1000
                        );
                        couponDataToProcess.usedOrderId = null;
                        Coupon.saveData(couponDataToProcess, function(
                          err,
                          couponDataReceived
                        ) {
                          if (err) {
                      ...
                      Order.findOneAndUpdate(
                        ...
                        },
                        {
                          $set: {
                            discountCouponId:
                              allData.selectedDiscount.coupon._id
                          }
                        },
                        {
                          new: true
                  ...
                  model.generateInvoiceOrOrderYear("OR", function(
                    err,
                    orderYear
                  ) {
  ...
  getUserOrders: function(data, callback) {
    Order.find({
      ...
      .deepPopulate(
        "products.product courierType products.product.size products.product.color"
      )
      .sort({
        createdAt: -1
      })
                ...
                async.waterfall(
                  [
                    ...
                    function deductQuantity(foundProduct, cbSubWaterfall1) {
                      var deductPrice =
                        _.round(foundProduct.price) * product.quantity;
                      if (data.return) {
        ...
        function getOrders(user, cbWaterfall1) {
          ...
          if (user._id == data.user) {
            ...
            Order.find({
              ...
              .deepPopulate(
                "returnedProducts.product order._id returnedProducts.product.size returnedProducts.product.color"
              )
              .exec(function(err, orders) {
  ...
  ConfirmOrderPlacedMail: function(data, callback) {
    User.findOne({
      ...
      if ((error, created == undefined)) {
        console.log(
          "User >>> ConfirmOrderPlaced >>> User.findOneAndUpdate >>> error",
          error
        );
        callback(error, null);
      } else {
        Order.findOne({
          ...
          .deepPopulate("products.product products.product.size")
          .lean()
          .exec(function(error, orderss) {
            ...
            if ((error, orderss == undefined)) {
              ...
              _.each(emailData.order, function(n) {
                emailData.cartAmount =
                  emailData.cartAmount + _.round(n.product.price) * n.quantity;
              });
              ...
              _.each(emailData.order, function(n) {
                if (n.discountAmount || n.discountPriceOfProductApplied) {
                  emailData.discount =
                    emailData.discount +
                    _.round(n.discountAmount) +
                    _.round(n.discountPriceOfProductApplied);
                  emailData.dis =
                    emailData.dis + _.round(n.discountPriceOfProductApplied);
                  emailData.disAfter =
                    emailData.disAfter + _.round(n.discountAmount);
                } else {
                  emailData.discount += 0;
                  emailData.dis += 0;
                  emailData.disAfter += 0;
              ...
              Config.ConfirmOrderPlacedMail(emailData, function(err, response) {
                if (err) {
                  ...
                  Config.ConfirmOrderPlacedMail(emailData, function(
                    err,
                    response
                  ) {
                    if (err) {
  ...
  returnedProductEmail: function(data, callback) {
    ...
    User.findOne({
      ...
      if ((error, created == undefined)) {
        console.log(
          "User >>> returnedProductEmail >>> User.findOneAndUpdate >>> error",
          error
        );
        callback(error, null);
      } else {
        Order.findOne({
          ...
          .deepPopulate(
            "returnedProducts.product returnedProducts.product.size"
          )
          .lean()
          .exec(function(error, orderss) {
  ...
  cancelProductEmail: function(data, callback) {
    console.log(
      "User >>> cancelProductEmail >>> User.findOneAndUpdate >>> data",
      data
    );
    User.findOne({
      _id: data._id
      ...
      if ((error, created == undefined)) {
        console.log(
          "User >>> cancelProductEmail >>> User.findOneAndUpdate >>> error",
          error
        );
        callback(error, null);
      } else {
  ...
  giftVoucherCodeMail: function(data, callback) {
    User.findOne({
      ...
      if ((error, created == undefined)) {
        console.log(
          "User >>> giftVoucherCodeMail >>> User.findOneAndUpdate >>> error",
          error
        );
        callback(error, null);
      } else {
            ...
            if ((error, orderss == undefined)) {
              ...
              async.eachSeries(
                ...
                function(voucher, callback1) {
                  ...
                  Config.giftVoucherCodeMail(emailData, function(
                    err,
                    response
                  ) {
                    if (err) {
  ...
  generateInvoiceOrOrderYear: function(str, callback) {
    var today = new Date();
    var lastTwoDigitYear = today
      .getFullYear()
      .toString()
      .substr(2, 2);
    console.log("today", lastTwoDigitYear);
  ...
  generateInvoice: function(data, callback) {
    ...
    Order.findOne({
      ...
      .lean()
      .deepPopulate("products.product user")
      .exec(function(err, order) {
        _.each(order.products, function(product, index) {
          value = _.round(product.product.mrp) * product.quantity;
          discountPercent =
            (product.discountAmount * 100) / product.product.mrp;
        ...
        order.totalAmount = _.round(
          subTotal - totalDiscount + gst + order.shippingAmount
        );
  ...
  populateOrderData: function(data, callback) {
    Order.find({})
      .deepPopulate(
        "products.product products.product.size products.product.prodCollection products.product.color user courierType returnedProducts.product returnedProducts.product.size returnedProducts.product.color"
      )
      .lean()
      .exec(function(err, order) {
