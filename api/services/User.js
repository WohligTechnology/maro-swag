...
var model = {
  ...
  sendOTPEmail: function(created, callback) {
    console.log(created);
    var emailData = {};
    ...
    Config.email(emailData, function(err, response) {
      if (err) {
        ...
        sendData.verifyAcc = created.verifyAcc;
        callback(null, sendData);
      } else {
        callback("errorOccurredRegister", null);
      }
    ...
    if (!data._id) {
      ...
      User.findOneAndUpdate(
        ...
        if ((error, created == undefined)) {
          console.log(
            "User >>> registerUser >>> User.findOneAndUpdate >>> error",
            error
          );
          callback(error, null);
        } else {
          User.sendOTPEmail(created, callback);
        }
      });
    } else {
      ...
      if (err) {
        ...
        if (!_.isEmpty(data)) {
          if (!_.isEmpty(data.shippingAddresses)) {
            data.shippingAddress = JSON.parse(
              JSON.stringify(data.shippingAddresses[0])
            );
  ...
  sendForgotPasswordOtp: function(data, callback) {
    ...
    async.waterfall(
      ...
      function(err, result) {
        console.log(
          " ***** async.waterfall final response of sendForgotPasswordOtp ***** ",
          result
        );
        callback(err, result);
      }
  ...
  isUserLoggedIn: function(accessToken, callback) {
    User.findOne({
      ...
      .exec(function(err, user) {
        ...
        else if (!_.isEmpty(user)) callback(null, user);
        else
          callback(
            {
  ...
  updateUser: function(data, callback) {
    console.log(
      "data inside removeWishListFromUser: ",
        ...
        [
          ...
          function updateAddress(user, cbWaterfall1) {
            ...
            if (data.shippingAddress) {
              var idx = _.findIndex(user.shippingAddresses, function(
                userAddress
              ) {
                return User.checkShippingAddress(
                  userAddress,
  ...
  welcomeEmail: function(data, callback) {
    User.findOne({
      ...
      if ((error, created == undefined)) {
        console.log(
          "User >>> welcomeEmail >>> User.findOneAndUpdate >>> error",
          error
        );
        callback(error, null);
      } else {
