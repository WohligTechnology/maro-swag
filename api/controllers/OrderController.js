...
var controller = {
  ...
  redirectUrl: function(req, res) {
    if (req.body) {
      ...
      var body = "",
        workingKey = global.workingKey, //Put in the 32-Bit key shared by CCAvenues.
        encRequest = "";
      var ccavResponse = "";
      ...
      console.log(resJson);
      if (
        resJson.order_status === "Success" ||
        resJson.order_status === "Aborted"
      ) {
    ...
    function updateOrder(order, checkFlag) {
      Order.findOneAndUpdate(
        ...
        if (err) {
          ...
          res.redirect(env.realHost + "/error-msg/" + order.order_id);
        } else {
          if (checkFlag) {
            ...
            res.redirect(env.realHost + "/thankyou-msg/" + order.order_id);
          } else {
            //  redirect to sorry page
            res.redirect(env.realHost + "/error-msg/" + order.order_id);
          }
    ...
    Order.findOne({
      ...
      .exec(function(err, order) {
        var toPayment = {
          ...
          redirect_url: env.realHost + "/api/Order/redirectUrl",
          cancel_url: env.realHost + "/api/Cart/cancelUrl",
          language: "EN",
  ...
  postReq: function(req, res) {
    console.log(global.workingKey);
    var http = require("http"),
      fs = require("fs"),
      qs = require("querystring");
    var ccav = require("./ccavutil.js");
    var body = "",
      workingKey = "FB1609CBCCCAA12EE1F460922C470103", //Put in the 32-Bit key shared by CCAvenues.
      accessCode = "AVMT01EK30BR58TMRB", //Put in the Access Code shared by CCAvenues.
      encRequest = "",
      formbody = "";
  ...
  hdfcPaymentGateway: function(req, res) {
    ...
    var body = "",
      workingKey = global.workingKey, //Put in the 32-Bit key shared by CCAvenues.
      accessCode = global.accessCode, //Put in the Access Code shared by CCAvenues.
      encRequest = "",
      formbody = "";
