myApp.controller("MycartCtrl", function(
  $scope,
  myService,
  ModalService,
  $state,
  ...
  $scope.applyCouponSubmit = function(couponName, productId) {
    ...
    if (!productId && couponName) {
      NavigationService.getCoupon(couponName, function(couponData) {
        ...
        if (couponData.data.data == "Coupon Invalid") {
          ...
          if ($scope.mycartTable) {
            ...
            $scope.grandTotal = $scope.total = CartService.getTotal(
              $scope.mycartTable.products
            );
            $scope.grandTotalAfterDiscount = 0;
            if ($scope.validCoupon.cAmount) {
              ...
              $scope.grandTotalAfterDiscount =
                ($scope.total * $scope.validCoupon.percentage) / 100;
              if (
      ...
      if (!isEmptyObject($scope.discountSelected)) {
        ...
        console.log(
          "$scope.discountSelected in applyCouponSubmit",
          $scope.discountSelected.discountType
        );
        ...
        if (
          $scope.discountSelected.discountType == "59f06bc7647252477439a1e4"
        ) {
          ...
          if ($scope.grandTotal >= $scope.discountSelected.yValue) {
            ...
            console.log(
              "$scope.mycartTable.products$scope.mycartTable.products",
              $scope.mycartTable.products
            );
            ...
            $scope.discountSelected = {};
            alert(
              "Your Cart Total Low to Avail This Discount!!! Shop More to Get This Discount..."
            );
          }
        } else if (
          $scope.discountSelected.discountType == "59d329e334d0832185b7f577"
        ) {
          console.log(
            "Buy 1 Get x Off Buy 2 Get 2x Off",
            $scope.discountSelected
          );
          //for Buy 1 Get x Off Buy 2 Get 2x Off
          $.jStorage.deleteKey("coupon");
          $.jStorage.deleteKey("gifts");
          myService.getAllProductsByDiscount(
            $scope.discountSelected._id,
            function(data) {
              var totalCountProductsInB1GXOff = 0;
              ...
              _.each($scope.mycartTable.products, function(cartProduct) {
                ...
                _.each(sortedArray, function(product) {
                  if (product._id == cartProduct.product._id) {
                    ...
                    if (
                      $scope.discountSelected._id == "59ed87aa0a9eb11654a1eb87"
                    ) {
                      $scope.discountPriceOfProduct =
                        cartProduct.product.price * 0.2;
                    } else {
                      $scope.discountPriceOfProduct =
                        cartProduct.product.price * 0.1;
                    }
                    switch (cartProduct.quantity) {
                      case 1:
                        cartProduct.product.discountApplicable = true;
                        cartProduct.product.discountPriceOfProduct =
                          $scope.discountPriceOfProduct;
                        cartProduct.product.priceWithDiscount = $scope.priceWithDiscount =
                        ...
                        if ($scope.priceWithDiscount > 999) {
                          var unitPrice =
                            ((_.round(cartProduct.product.price) -
                              $scope.discountPriceOfProduct) *
                              100) /
                            (100 + 12);
                      ...
                      case 2:
                        ...
                        cartProduct.product.discountPriceOfProduct = $scope.discountPriceOfProduct =
                          $scope.discountPriceOfProduct * 2;
                        cartProduct.product.priceWithDiscount = $scope.priceWithDiscount =
                        ...
                        if ($scope.priceWithDiscount > 999) {
                          var unitPrice =
                            ($scope.priceWithDiscount * 100) / (100 + 12);
                          var tax = _.round(unitPrice) * 0.12;
                          // var tax = $scope.priceWithDiscount * 0.12;
                          cartProduct.product.gst = _.round(tax);
                        } else {
                          var unitPrice =
                            ($scope.priceWithDiscount * 100) / (100 + 5);
                          var tax = _.round(unitPrice) * 0.05;
                          // var tax = $scope.priceWithDiscount * 0.05;
                          cartProduct.product.gst = _.round(tax);
                        }
                      ...
                      case 3:
                        cartProduct.product.discountApplicable = true;
                        cartProduct.product.discountPriceOfProduct = $scope.discountPriceOfProduct =
                          $scope.discountPriceOfProduct * 3;
                        cartProduct.product.priceWithDiscount = $scope.priceWithDiscount =
                          cartProduct.product.price * cartProduct.quantity -
                          $scope.discountPriceOfProduct;
                        // $scope.grandTotalAfterDiscount = $scope.grandTotalAfterDiscount + $scope.discountPriceOfProduct;
                      ...
                      case 4:
                        cartProduct.product.discountApplicable = true;
                        cartProduct.product.discountPriceOfProduct = $scope.discountPriceOfProduct =
                          $scope.discountPriceOfProduct * 4;
                        cartProduct.product.priceWithDiscount = $scope.priceWithDiscount =
                          cartProduct.product.price * cartProduct.quantity -
                          $scope.discountPriceOfProduct;
                        // $scope.grandTotalAfterDiscount = $scope.grandTotalAfterDiscount + $scope.discountPriceOfProduct;
                      ...
                      case 5:
                        cartProduct.product.discountApplicable = true;
                        cartProduct.product.discountPriceOfProduct = $scope.discountPriceOfProduct =
                          $scope.discountPriceOfProduct * 5;
                        cartProduct.product.priceWithDiscount = $scope.priceWithDiscount =
                          cartProduct.product.price * cartProduct.quantity -
                          $scope.discountPriceOfProduct;
                        // $scope.grandTotalAfterDiscount = $scope.grandTotalAfterDiscount + $scope.discountPriceOfProduct;
                      ...
                      case 6:
                        cartProduct.product.discountApplicable = true;
                        cartProduct.product.discountPriceOfProduct = $scope.discountPriceOfProduct =
                          $scope.discountPriceOfProduct * 6;
                        cartProduct.product.priceWithDiscount = $scope.priceWithDiscount =
                          cartProduct.product.price * cartProduct.quantity -
                          $scope.discountPriceOfProduct;
                        // $scope.grandTotalAfterDiscount = $scope.grandTotalAfterDiscount + $scope.discountPriceOfProduct;
                      ...
                      case 7:
                        cartProduct.product.discountApplicable = true;
                        cartProduct.product.discountPriceOfProduct = $scope.discountPriceOfProduct =
                          $scope.discountPriceOfProduct * 7;
                        cartProduct.product.priceWithDiscount = $scope.priceWithDiscount =
                          cartProduct.product.price * cartProduct.quantity -
                          $scope.discountPriceOfProduct;
                        // $scope.grandTotalAfterDiscount = $scope.grandTotalAfterDiscount + $scope.discountPriceOfProduct;
                      ...
                      default:
                        ...
                        $scope.addDis += _.round($scope.discountPriceOfProduct);
                        if (
                          cartProduct.product.price != cartProduct.product.mrp
                        ) {
                          if (cartProduct.product.price > 999) {
                            var unitPrice =
                              ($scope.priceWithDiscount * 100) / (100 + 12);
              ...
              $scope.grandTotalAfterDiscount =
                $scope.grandTotalAfterDiscount + $scope.addDis;
              $scope.discountValueObject = {
                discountAmount: $scope.grandTotalAfterDiscount,
                grandTotalAfterDiscount:
                  $scope.total - $scope.grandTotalAfterDiscount,
                grandTotalAfterGst: $scope.productGst,
                gst: $scope.productGst,
                selectedDiscount: $scope.discountSelected,
                totalAmountOfOrder: $scope.total,
          ...
          $scope.discountSelected.discountType == "59ede2fcd30c7e2ab3324ece"
        ) {
          ...
          if ($scope.grandTotal >= $scope.discountSelected.yValue) {
            $scope.grandTotal =
              $scope.grandTotal - $scope.discountSelected.xValue;
            // $scope.grandTotalAfterDiscount = $scope.discountSelected.xValue;
            $scope.grandTotalAfterDiscount =
              $scope.grandTotalAfterDiscount + $scope.discountSelected.xValue;
          ...
          $scope.discountSelected.discountType == "59e44d48f255331e48fc428f"
        ) {
          $.jStorage.deleteKey("coupon");
          $.jStorage.deleteKey("gifts");
          //gifts section
          ...
          $scope.discountSelected.discountType == "59f1e3327ddd0f0dcda6fe6f"
        ) {
          ...
          myService.getAllProductsByDiscount(
            ...
            function(data) {
              ...
              console.log(
                "allDiscountedProcuctsTotalQty",
                allDiscountedProcuctsTotalQty
              );
              var seperateProductsInBOGOOfferLength =
                seperateProductsInBOGOOffer.length;
              var processingArray = seperateProductsInBOGOOffer;
              var sumOfPricesToBeDiscard = Math.floor(
                seperateProductsInBOGOOfferLength / 2
              );
              //  console.log("seperateProductsInBOGOOffer%%%",seperateProductsInBOGOOffer);
              var descardedProductindex =
                seperateProductsInBOGOOfferLength - sumOfPricesToBeDiscard;
              console.log("descardedProductindex", descardedProductindex);
              var totalDiscount = 0;
              var iteration = 0;
              $scope.productGst = 0;
              var finalArray = seperateProductsInBOGOOffer.slice(
                0,
                -sumOfPricesToBeDiscard
              );
              var uniqueProducts = [];
              for (
                i = descardedProductindex - 1;
                i < seperateProductsInBOGOOfferLength - 1;
                i++
              ) {
                  ...
                  if (uniqueProducts[i] != current) {
                    ...
                    _.each($scope.mycartTable.products, function(cartProduct) {
                      if (cartProduct._id == current._id) {
                        cartProduct.product.discountApplicable = true;
                        cartProduct.product.discountPriceOfProduct =
                          cartProduct.product.price * cnt;
                        cartProduct.product.priceWithDiscount =
                ...
                _.each(finalArray, function(val) {
                  _.each(seperateProductsInBOGOOffer, function(val1) {
                    ...
                    if (val1.product._id == val.product._id) {
                      ...
                      val.product.priceWithDiscount =
                        val.product.price * val.quantity - val.product.price;
                      if (val.product.discountPriceOfProduct > 999) {
                        var unitPrice =
                          ((val.product.price -
                            val.product.discountPriceOfProduct) *
                            100) /
                          (100 + 12);
              ...
              if (finalArray) {
                ...
                _.each(finalArray, function(last) {
                  ...
                  var unitPrice =
                    ((last.product.price * last.quantity -
                      last.product.discountPriceOfProduct) *
                      100) /
                    (100 + 12);
                  var tax = unitPrice * 0.12;
              ...
              if ($scope.mycartTable) {
                ...
                _.each($scope.mycartTable.products, function(cartProduct) {
                  $scope.productGst =
                    $scope.productGst + cartProduct.product.gst;
                });
                //  $.jStorage.set("grandTotal", $scope.grandTotal);
              }
              ...
              $scope.grandTotal =
                $scope.grandTotal - $scope.grandTotalAfterDiscount;
          ...
          myService.getAllProductsByDiscount(
            ...
            function(data) {
              ...
              _.each(data.products, function(product) {
                _.each($scope.mycartTable.products, function(cartProduct) {
                  if (product._id == cartProduct.product._id) {
                    totalCountProductsInB1GXOff =
                      totalCountProductsInB1GXOff + cartProduct.quantity;
                  }
                });
              });
              if (totalCountProductsInB1GXOff > 1) {
                ...
                $scope.grandTotal =
                  $scope.grandTotal - $scope.discountSelected.maxDiscountAmount;
                $scope.grandTotalAfterDiscount =
                  $scope.discountSelected.maxDiscountAmount;
      ...
      _.forEach($scope.mycartTable.products, function(value) {
        ...
        if (value.product.price != value.product.mrp) {
          if (value.product.price > 999) {
            var discount = 0;
            var unitPrice =
              (_.round(value.product.price) * value.quantity * 100) /
              (100 + 12);
            var tax = _.round(unitPrice) * 0.12;
            value.product.gst = _.round(tax);
            value.gst = _.round(tax);
            value.product.discountApplicable = false;
            value.product.discountPriceOfProductApplied = discount =
              (value.product.mrp - value.product.price) * value.quantity;
            ...
            var discount = 0;
            var unitPrice =
              (_.round(value.product.price) * value.quantity * 100) / (100 + 5);
            var tax = _.round(unitPrice) * 0.05;
            value.product.gst = _.round(tax);
            value.gst = _.round(tax);
            value.product.discountApplicable = false;
    ...
    _.forEach($scope.mycartTable.products, function(value) {
      if (value.product.price != value.product.mrp) {
        if (value.product.price > 999) {
          var discount = 0;
          var unitPrice =
            (_.round(value.product.price) * value.quantity * 100) / (100 + 12);
          var tax = _.round(unitPrice) * 0.12;
      ...
      $scope.productGst = _.round(
        $scope.checkNAN($scope.productGst) + value.product.gst
      );
      $scope.afterDis += value.product.discountPriceOfProduct;
      $scope.grandTotalAfterDiscount = $scope.afterDis;

      $scope.grandTotalAfterDiscount = _.isNaN($scope.grandTotalAfterDiscount)
        ? 0
        : $scope.grandTotalAfterDiscount +
          value.product.discountPriceOfProduct +
          value.product.discountPriceOfProductApplied;
  ...
  $scope.checkStockStatus = function(prod) {
    if (prod.quantity > prod.product.quantity) {
      $scope.flag++;
      angular.element(
        document.getElementsByClassName("checkout--btn")
      )[0].disabled = true;
  ...
  $scope.redirectToCheckot = function() {
    console.log(
      "$scope.discountApplicableforCart",
      $scope.discountApplicableforCart
    );
    var cart = {};
