myApp
  ...
  .controller("wishlistModalCtrl", function (
    $scope,
    $state,
    $uibModalInstance,
    UserService,
  ...
  .controller("loginModalCtrl", function (
      ...
      UserService.login($scope.loginData, function (data) {
        if (!_.isEmpty(data.data.data)) {
          ...
          if ($scope.userData.verifyAcc) {
            ...
            $scope.accessToken =
              $scope.userData.accessToken[
                $scope.userData.accessToken.length - 1
              ];
    ...
    $scope.registerUser = function () {
      UserService.userRegistration($scope.formData, function (data) {
        ...
        if (data.data.error) {
          $scope.errormsg =
            "User already exists with the given emailId.Please login to proced";
      ...
      if (otp) {
        ...
        UserService.verifyRegisterUserWithOtp($scope.userData, function (data) {
          ...
          if (data.data.value === true) {
            ...
            if (data.data.error == "otpNoMatch") {
              $scope.errorMessage = "Invalid OTP. Please provide valid OTP.";
            } else if (data.data.error == "otpExpired") {
              $scope.errorMessage =
                "OTP Expired. Please click on 'Resend OTP'!";
              $scope.resendOtpBut = true;
            } else if (data.data.error == "noOtpFound") {
              $scope.errorMessage =
                "No OTP found. Please click on 'Resend OTP'!";
              $scope.resendOtpBut = true;
            } else {
              $scope.errorMessage =
